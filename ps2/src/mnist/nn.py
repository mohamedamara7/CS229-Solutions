...
def get_initial_params(input_size, num_hidden, num_output):
    ...
    # *** START CODE HERE ***
    dic={'W1':np.random.normal(size = (input_size,num_hidden)),#np.random.normal(0,np.sqrt(2/(num_hidden+input_size)),(input_size,num_hidden))
         'b1':np.zeros(num_hidden),
         'W2':np.random.normal(size = (num_hidden,num_output)),#np.random.normal(0,np.sqrt(2/(num_output+num_hidden)),(num_hidden,num_output))
...
def forward_prop(data, labels, params):
    ...
    a_2 = softmax(z_2)
    # predictions = np.argmax(a_2)
    avg_loss = -np.mean(np.sum(labels*np.log(a_2),axis=1))
    return (a_1,a_2,avg_loss)
    # *** END CODE HERE ***
...
def backward_prop_regularized(data, labels, params, forward_prop_func, reg):
    ...
    """
    # *** START CODE HERE ***
    forward = forward_prop_func(data, labels, params)
    dj_dz2 = (forward[1]-labels)
    dj_da1 = dj_dz2@params['W2'].T
    dj_dz1 = (forward[0]*(1-forward[0]))*dj_da1

    grad={'W2':forward[0].T@dj_dz2 / data.shape[0] + 2*reg*params['W2'],
         'b2':np.sum(dj_dz2,axis=0) / data.shape[0],
         'W1':data.T@dj_dz1 / data.shape[0] + 2*reg*params['W1'],
         'b1':np.sum(dj_dz1,axis=0) / data.shape[0]
         }
...
def gradient_descent_epoch(train_data, train_labels, learning_rate, batch_size, params, forward_prop_func, backward_prop_func):
    ...
    for batch in range(len(train_data)//batch_size):
        ...
        cur_batch_data = train_data[start_idx:start_idx+batch_size]
        cur_batch_labels = train_labels[start_idx:start_idx+batch_size]
        grad = backward_prop_func(cur_batch_data, cur_batch_labels, params, forward_prop_func)
