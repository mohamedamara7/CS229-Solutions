...
def run_em(x, w, phi, mu, sigma):
    ...
    while it < max_iter and (prev_ll is None or np.abs(ll - prev_ll) >= eps):
        ...
        phi, mu, sigma = m_step(x, w, phi, mu, sigma)
        # (3) Compute the log-likelihood of the data to check for convergence.
        # By log-likelihood, we mean `ll = sum_x[log(sum_z[p(x|z) * p(z)])]`.
        # We define convergence by the first iteration where abs(ll - prev_ll) < eps.
        # Hint: For debugging, recall part (a). We showed that ll should be monotonically increasing.
        prev_ll = ll
        ll = log_likelihood(x, w, phi, mu, sigma)
        it += 1
...
def run_semi_supervised_em(x, x_tilde, z_tilde, w, phi, mu, sigma):
    ...
    while it < max_iter and (prev_ll is None or np.abs(ll - prev_ll) >= eps):
        ...
        prev_ll = ll
        ll = log_likelihood(x, w, phi, mu, sigma)
        ll += alpha * log_likelihood(x_tilde, w, phi, mu, sigma, z_tilde)
        it += 1
        print('[iter: {:03d}, log-likelihood: {:.4f}]'.format(it, ll))
        # *** END CODE HERE ***
...
# *** START CODE HERE ***
# Helper functions
def prior(x, phi, mu, sigma):
    return (1/(np.sqrt(np.power(2*np.pi,len(x))*np.linalg.det(sigma))))*np.exp(-0.5*(x-mu).T@(np.linalg.inv(sigma))@(x-mu))*phi
def e_step(x, w, phi, mu, sigma):
    n, d = x.shape
    for i in range(n):
        sum = 0
        for j in range(K):
            temp = prior(x[i], phi[j], mu[j], sigma[j])
            w[i,j] = temp
            sum += temp
        w[i] = w[i]/sum
    return w
...
def m_step(x, w, phi, mu, sigma):
    ...
    for j in range(K):
        mu[j] = np.sum(w[:,j:j+1]*x,axis=0)/np.sum(w[:,j]) 
        sigma[j] = np.zeros_like(sigma[j])
        for i in range(n):
            diff = x[i]-mu[j]
            sigma[j] += w[i,j]*np.outer(diff, diff)
        sigma[j] = sigma[j]/(phi[j]*n)
    return phi, mu, sigma

def m_step_ss(x, x_tilde, z_tilde, w, phi, mu, sigma, alpha):
    n, d = x.shape
    n_hat, d_hat = x_tilde.shape
    k_counts = [np.sum(z_tilde == j) for j in range(K)]
    for j in range(K):
        phi[j] = (np.sum(w[:,j], axis=0) + alpha*k_counts[j])/(n+alpha*n_hat)
        mu[j] = (np.sum(w[:,j:j+1]*x,axis=0)+alpha*np.sum(x_tilde[(z_tilde == j).squeeze(), :], axis=0))/(np.sum(w[:,j])+alpha*k_counts[j]) 
        sigma[j] = np.zeros_like(sigma[j])
        for i in range(n):
            diff = x[i]-mu[j]
            sigma[j] += w[i,j]*np.outer(diff, diff)
        for i in range(n_hat):
            if z_tilde[i]==j:
                diff = x_tilde[i]-mu[j]
                sigma[j] += alpha*np.outer(diff, diff)
        sigma[j] = sigma[j]/((phi[j]*n)+alpha*k_counts[j])
    return phi, mu, sigma

def log_likelihood(x, w, phi, mu, sigma, z_tilde=None):
    n, d = x.shape
    ll = 0
    if z_tilde is None:
        #UnSupervised
        for i in range(n):
            p_x = 0
            for j in range(K):
                p_x += prior(x[i], phi[j], mu[j], sigma[j])
            ll += np.log(p_x)
    else:
        #Semi-Supervised
        n,d = z_tilde.shape
        for i in range(n):
            j = int(z_tilde[i])
            ll += np.log(prior(x[i], phi[j], mu[j], sigma[j]))
    return ll
# *** END CODE HERE ***
